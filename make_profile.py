...
def make_profile_image(name,server):
    try:
        def_result=make_profile(name,server)
    except:
...
def show_profile(name,server):
    threading.Thread(target=make_profile_image,args=(name,server),daemon=True).start()
...
#make_profile('던클','디레지에')
#show_profile('던클','디레지에')
